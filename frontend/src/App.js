...
const closePosition = async (positionId, reason = 'MANUAL') => {
  ...
  if (position) {
    ...
    try {
      ...
      if (data.success) {
        // Use backend returned balance (INR) if available
        let newBalance;
        if (data.user?.paperBalance !== undefined) {
          newBalance = data.user.paperBalance;
        } else {
          // Fallback: correctly convert USD to INR
        ...
        setEquity(newBalance + (totalPnl - pnl) * dollarRate); // also adjust equity
        ...
        // Update challenge stats and check rules
        ...
        // Optionally sync with backend to ensure consistency
        await syncUserWallet();
      }
    } catch (error) {
